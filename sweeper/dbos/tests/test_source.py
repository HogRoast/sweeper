...
from sweeper.dbos.source import Source, SourceKeys, SourceValues
...
class TestSource(TestCase):
    ...
    def test_create(self):
        obj = Source.create((98, 'source name TD', 'source fixtures_url TD', 'source url TD'))
    ...
    def test_select(self):
        ...
        objs = TestSource.db.select(Source.createAdhoc({'name': 'source name TD', 'fixtures_url': 'source fixtures_url TD', 'url': 'source url TD'}))
        self.assertEqual(len(objs), 1)
        self.assertEqual(objs[0].getId(), 98)
