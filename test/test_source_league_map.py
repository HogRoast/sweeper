...
from Footy.src.database.source_league_map import Source_League_Map, Source_League_MapKeys, Source_League_MapValues
...
class TestSource_League_Map(TestCase):
    ...
    def test_keys_adhoc(self):
        l = Source_League_Map.createAdhoc(AdhocKeys(None))
        self.assertEqual(l.getTable(), 'source_league_map')
        self.assertTrue(l._keys.getFields() is None)

    def test_createSingle(self):
        obj = Source_League_Map.createSingle((98, 'league mnemonic TD', 'source_league_map moniker TD'))
    ...
    def test_createMulti(self):
        rows = [(98, 'league mnemonic TD', 'source_league_map moniker TD'),
                (99, 'league mnemonic TD2', 'source_league_map moniker TD2')]
        objs = Source_League_Map.createMulti(rows)
    ...
    def test_select(self):
        ...
        objs = TestSource_League_Map.db.select(Source_League_Map.createAdhoc(AdhocKeys({'moniker': 'source_league_map moniker TD'})))
        self.assertEqual(len(objs), 1)
        self.assertEqual(objs[0].getSource_Id(), 98)
        self.assertEqual(objs[0].getLeague(), 'league mnemonic TD')
